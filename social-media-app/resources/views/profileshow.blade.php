                ...
                <div class="m-2">
                    ...
                    <div
                        ...
                        <div>
                            ...
                            <!-- Blade Template -->
                            @auth
                            <button class ="flex" onclick="like({{$post->post_id}})">
                                <span class="heart " id="{{$post->post_id}}">🤍</span>
                                <p class="text-white pl-3" id="count-{{$post->post_id}}">{{$post->like_count}}</p>
                            </button>
                        ...
                        <div class=" m-3 grid gap-2 sm:grid-cols-2">
                            <a href="{{url('/')}}/post/delete/{{$post->post_id}}">
                            <x-danger-button class="ml-3"> Delete Post </x-danger-button></a>

                            <x-primary-button x-data="" x-on:click.prevent="$dispatch('open-modal', 'edit-post-{{$post->post_id}}')">Edit Post</x-primary-button>
                            <x-modal name="edit-post-{{$post->post_id}}" :show="$errors->userDeletion->isNotEmpty()" focusable>
                                <form action="{{url('/')}}/post/edit/{{$post->post_id}}" method="POST" class="m-2" enctype="multipart/form-data">
                                    @csrf
                                    @method('patch')
                                    <div class='m-2'>
                                        <x-input-label for="name" value="Update Post" />
                                        <x-text-input type="text" name="post_text" class="mt-1 block w-full" value="{{$post->postText}}" id="name"
                                            autofocus />
